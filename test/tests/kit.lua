...
test("kit: bar for a person, a place, a faction and a campaign page", "dm", function()
  ...
  eq(list(buttonsOf(warden.html)), "Reveal | Mark met | Mark dead…")
...
test("kit: mark met from the bar", "dm", function()
  ...
  has(lastNotification().message, "The Warden: met in session 1, and revealed.")
  local bar = gm.bar()
  eq(list(buttonsOf(bar.html)), "Unreveal | Mark dead… | Unmark met")
